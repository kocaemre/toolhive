...
1. **Any allowlisted actor authorizes any ToolHive client.** The allowlist
   ...
   compared against the authenticated ToolHive client ID. Every ToolHive
   confidential client holding the token-exchange grant is delegation-equivalent
   with respect to an allowlisted external actor. Keeping that client set
   minimal is the operator's real control.
